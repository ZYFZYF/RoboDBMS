...
select max(O_CUSTKEY) as MAX_VALUE, min(O_CUSTKEY) as MIN_VALUE, avg(O_CUSTKEY) as AVG_VALUE_1, count(O_CUSTKEY) as COUNT_VALUE, sum(O_CUSTKEY) / count(O_CUSTKEY) as AVG_VALUE_2 from test_group;
