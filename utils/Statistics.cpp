...
//
// We are assured by the StatisticMgr that psKey_ is not a nullptr pointer.
//
...
RC StatisticsMgr::Register(const char *psKey, const Stat_Operation op,
    ...
    if (psKey == nullptr || (op != STAT_ADDONE && piValue == nullptr))
...
// will return nullptr.  The caller must remember to delete the memory
// returned when done.
//
