...
//
// File:        pf_BufferManager.cc
// Description: PF_BufferManager class implementation
...
PF_BufferManager::PF_BufferManager(int _numPages) : hashTable(PF_HASH_TBL_SIZE) {
...
// Ret:  PF return code
//
RC PF_BufferManager::GetPage(int fd, PageNum pageNum, char **ppBuffer,
                             int bMultiplePins) {
    RC rc;     // return code
...
// Ret:  PF return code
//
RC PF_BufferManager::AllocatePage(int fd, PageNum pageNum, char **ppBuffer) {
    RC rc;     // return code
    int slot;   // buffer slot where page is located
...
//
RC PF_BufferManager::FlushPages(int fd) {
    RC rc, rcWarn = OK_RC;  // return codes
...
//
RC PF_BufferManager::PrintBuffer() {
...
//       is called.
RC PF_BufferManager::ClearBuffer() {
    RC rc;
...
//
RC PF_BufferManager::ResizeBuffer(int iNewSize) {
    int i;
    RC rc;
...
// buffer pool.
//
RC PF_BufferManager::GetBlockSize(int &length) const {
    length = pageSize;
    return OK_RC;
}
...
// user.
//
RC PF_BufferManager::AllocateBlock(char *&buffer) {
    RC rc = OK_RC;
